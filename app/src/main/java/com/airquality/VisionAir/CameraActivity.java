...
//            //Toast.makeText(getApplicationContext(), "Bitmap", Toast.LENGTH_SHORT).show();
            ...
            Log.i(TAG, "feature weather: " + features[0][3]);

            feature_check = 1;
            if (features[0][3] != -9999) {
                Intent intent = new Intent(CameraActivity.this, Predict_Train.class);
                ...
                for (int i = 0; i < features.length; i++) {
                    for (int j = 0; j < features[i].length; j++) {
                        Log.i(TAG, "camera features " + i + " " + j + ": " + features[0][j]);
                    }
                }
                feature_check = 0;
...
//        if(tem_flag==0) {
    ...
    public void onProviderDisabled(String provider) {
        //Toast.makeText(CameraActivity.this, "Please Enable GPS and connect to the Internet", Toast.LENGTH_SHORT).show();
    }
...
//        DatabaseReference myRef = database.getReference("message");
        ref = FirebaseDatabase.getInstance().getReference().child("APIData");
...
//    }
    ...
    void fetch_prev() {
        keyDate = "26-08-2019 18:46:21";
        Log.i(TAG, "START fetch" + keyDate);
//        ref = FirebaseDatabase.getInstance().getReference(keyDate).child("records");
        ref = FirebaseDatabase.getInstance().getReference().child("APIData").child(keyDate).child("records");
